...
var vue = new Vue({
  ...
  mounted(){
      ...
      var labelurl = '/crm/cbCooperation/getList';
      var clienturl = '/crm/customer/getList';
      var lxrurl = '/crm/contacts/getList';
      var url = this.vueUrl();
      this.geturldata();//获取url参数信息
      this.saleChance(url+crm_url); //销售机会
      this.clientName(url+clienturl);
      this.visitWay(url+crm_url); //跟进方式 
      this.customerData(url+lxrurl); //联系人信息
      var urlus = url+'/tsuser/getTSUsers?pageSize=-1';
      this.request(urlus); //请求用户信息 
      this.localdata(); //获取table显示与隐藏   
  },
  data:{
    form:{
        typeId:''
